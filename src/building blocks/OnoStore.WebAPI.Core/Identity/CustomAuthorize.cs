...
namespace OnoStore.WebAPI.Core.Identity
{
    ...
    public class RequirementClaimFilter : IAuthorizationFilter
    {
        ...
        public void OnAuthorization(AuthorizationFilterContext context)
        {
            ...
            {
                context.Result = new StatusCodeResult(401); // I don't know who you are
                return;
            }

            if (!CustomAuthorization.ValidateUserClaims(context.HttpContext, _claim.Type, _claim.Value))
            {
                context.Result = new StatusCodeResult(403); // I know you but you are not authorized to access
            }
        }
    }
}
